                ...
                <div class="container-fluid">
                    <div class="row" style="padding-top: 15px">
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" onsubmit="return confirm('Are you sure you want to submit?')">
                            ...
                            <div class="row mb-3">
                                <label for="incomeInput" class="col-sm-2 col-form-label text-white">Income</label>
                                <div class="col-sm-10">
                                <input type="text" class="form-control bg-dark text-white" id="incomeInput" name="income">
                                </div>
                            </div>
                            <div class="row mb-3">
                        ...
                        <?php 
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                echo '<th class="text-white">Income</th>';
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo '<td contenteditable="true" class="text-white" data-id="' . $row['id'] . '" data-column="income">' . htmlspecialchars($row['income']) . '</td>';
                                    echo '<td contenteditable="true" class="text-white" data-id="' . $row['id'] . '" data-column="date">' . htmlspecialchars($row['date']) . '</td>';
                                    echo "<td>";
                                    echo "<form method='POST' onsubmit=\"return confirm('Are you sure you want to delete?')\" action='delete_income.php'>";
...
$(document).ready(function() {
    $('td[contenteditable="true"]').on('blur', function() {
        ...
        $.ajax({
            url: 'update_income.php',  // PHP script to update the database
            type: 'POST',
            data: {
                id: id,
                column: column,
