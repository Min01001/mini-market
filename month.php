...
$sql = "SELECT product, SUM(quantity) AS total_quantity, price ,SUM(total) AS total_price, image, DATE_FORMAT(date, '%Y-%m') AS month, id FROM sell GROUP BY month, product ORDER BY id DESC, product";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<td class="text-danger">' . $row['month'] . '</td>';
