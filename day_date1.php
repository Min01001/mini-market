                    ...
                    <?php
                            if ($dropdownResult->num_rows > 0) {
                                while ($dropdownRow = $dropdownResult->fetch_assoc()) {
                                    $selected = ($selectedProduct === $dropdownRow['date']) ? 'active' : '';
                                    echo '<li><a class="dropdown-item ' . $selected . '" href="?date=' . urlencode($dropdownRow['date']) . '">' . htmlspecialchars($dropdownRow['date']) . '</a></li>';
                                }
                            } else {
